...
- The **backend does NOT change**. It is a Cloudflare Worker that proxies to a self-hosted Mac-mini Bun server. You are building a **new native client against the exact same HTTP API** (base origin `https://music.streamarena.xyz`).
...
3. **API origin + relative URLs.** Everything goes to `https://music.streamarena.xyz`. The web app had a `capacitor://` URL-rewriting shim (`src/lib/native-api.ts`, `src/client/native-network.ts`) — **delete that concept entirely**; in RN just use the absolute base origin. Media URLs come back relative → prepend the base before handing to the player/image.
...
```
Expo RN app  ──HTTPS──>  Cloudflare Worker (music.streamarena.xyz)  ──>  Mac-mini Bun server
